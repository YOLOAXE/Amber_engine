...
Terrain::Terrain(unsigned int _size, float _tileSize, FastNoise::NoiseType _noiseType, float _noiseFrequency) :
...
Terrain::Terrain(unsigned int _size, float _tileSize, FastNoise::NoiseType _noiseType, float _noiseFrequency, int _seed) :
...
Terrain::Terrain(unsigned int _size, float _tileSize, FastNoise::NoiseType _noiseType, float _noiseFrequency, int _seed, int _noiseOctaves, int _noiseMagnitude, bool _isIsland) :
...
void Terrain::makeIsland()
{
	...
	for (int row = 0; row < width; row++)
	{
		...
		unsigned int rowIndexOffset = row * width * step;
		for (int col = 0; col < height; col++) {
			// find the current index offset for the column
			unsigned int colIndexOffset = col * step;

			// find the index for the vertex data
			unsigned int vertexStartIndex = rowIndexOffset + colIndexOffset;

			// find X and Y distance from the center
			float diffX = fabsf(centerX - col);
			float diffY = fabsf(centerY - row);

			// calculate normalised distance
			float distance = sqrtf(pow(diffX, 2) + pow(diffY, 2));
			float distanceNrm = distance / maxDistance;

			// fetch current value
			float y = vertices[vertexStartIndex + 1];
...
void Terrain::render(unsigned int& program)
{
	/*glBindVertexArray(VAO);

	// draw fill colour
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 9, (void*)(sizeof(float) * 6));
...
void Terrain::updateHeightmap(bool useNewSeed)
{
	// step for each vertex data set
	const int step = 9;
	...
	for (int row = 0; row < width; row++)
	{
		...
		for (int col = 0; col < height; col++) {
			...
			float y = noise.GetNoise(col, row) * magnitude;
			y = pow(y, exponent);
...
void Terrain::generateVertices()
{
	...
	for (int row = 0; row < width; row++)
	{
		...
		float rowOffset = row * tileSize;
		for (int col = 0; col < height; col++) {

			// position data
			// calculate height data - use FastNoise and then apply magnitude / exponent modifications
			float y = magnitude * noise.GetNoise(col, row);
			y = pow(y, exponent);

			// add positional data to vertex array
			vertices.push_back((float)col * tileSize);
			vertices.push_back(y);
			vertices.push_back((float)rowOffset);
...
void Terrain::updateColourForHeight(unsigned int& startIndex, float& y)
{
	if (y < 0.06) {
...
void Terrain::addColourForHeight(float& y)
{
	if (y < 0.06) {
